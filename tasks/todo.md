...
- [ ] Task 29: recycler-service event-infrastructure schema
  ...
  - **Acceptance criteria:**
    ...
    - [ ] `OutboxEventEntity`/adapter (mirrors Task 27's shape, **including its `findPending` query's `ORDER BY e.createdAt ASC, e.id ASC` tiebreaker** — user-caught in Task 27's review, applies identically here); `CollectionRegisteredLedgerEntity`/repo
    ...
    - [ ] `AssociationJpaRepository.incrementTotalKilos(UUID, BigDecimal)` — `@Modifying(clearAutomatically = true) @Query("UPDATE ... SET total_kilos_collected = total_kilos_collected + :amount ...")`, never a load-mutate-save. `clearAutomatically = true` isn't optional polish: found in Task 27 that a bulk `@Modifying` UPDATE bypasses Hibernate's first-level cache, so a later read sharing the same transaction (very plausible here, e.g. a future read of `Association.totalKilosCollected` right after an increment) would otherwise see a stale cached value. Same requirement for the outbox `updateStatus` query this task also adds — both need `@Transactional` too, for the same reason Task 27's did (`@Modifying` isn't transactional just by being annotated)
  - **Verification:**
    - [ ] IT tests: ledger table round-trip, outbox adapter round-trip, `findPending`'s tiebreaker (mirror Task 27's `tiedCreatedAtOrdersByIdAsATiebreaker` test)
