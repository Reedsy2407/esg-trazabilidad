...
- **`spring-boot-starter-oauth2-resource-server` is declared `scope=provided` in shared-kernel's own `pom.xml`, not normal `compile` scope.** Discovered mid-Task-62: the starter's mere *presence* on a service's classpath (regardless of whether any bean actually uses it) is enough for Spring Boot's own `SecurityAutoConfiguration` to install a default lockdown (HTTP Basic + a generated password, then a plain 403 once a `JwtDecoder` bean also exists) on every endpoint — confirmed directly against `recycler-service`'s existing `@WebMvcTest` suite, which started failing the moment this dependency was added at normal scope to shared-kernel, *before* `JwtSecurityConfig` was even registered anywhere. `provided` scope lets shared-kernel compile `JwtSecurityConfig` against these classes without propagating the dependency transitively to every consumer; each service that actually wants JWT support (`auth-service` now, `recycler-service`/`collection-service`/`reporting-service` in Tasks 66-68) re-declares it explicitly in its own `pom.xml` at normal scope, at the same moment it adds its own `SecurityFilterChain` bean — so Spring Boot's default never gets a chance to apply.
...
Task 61: Admin bootstrap runner (SecureRandom password, logged once, gated on table emptiness)
    │
    ▼
Task 62: shared-kernel JwtSecurityConfig (JwtDecoder, JwtAuthenticationEntryPoint/
...
- [x] Task 59: auth-service scaffolding (new Maven module, `pom.xml`, `application.yml` port 8084, empty Liquibase master changelog) — no security config, no entities yet
...
- [x] Task 60: `StaffUser` persistence (Liquibase `v0.1.0` with a unique index on `email`, `StaffUserEntity`/`JpaRepository`/`RepositoryAdapter`, `BCryptPasswordEncoder` bean) — no `role` field, per Resolved Decisions
- [x] Task 61: Admin bootstrap runner (`ApplicationRunner`; if `staff_user` is empty AND `ADMIN_BOOTSTRAP_EMAIL` is set, generate a `SecureRandom` password, bcrypt-hash it, persist the account, log the plaintext once at WARN — never on a later restart, never from an env-supplied password)
- [x] Task 62: shared-kernel `JwtSecurityConfig` (`JwtDecoder` bean off `${JWT_SECRET}`, `JwtAuthenticationEntryPoint`/`JwtAccessDeniedHandler` shaping 401/403 in the same `{code, detail}`-carrying `ProblemDetail`-shaped JSON `GlobalExceptionHandler` already uses, hand-serialized since Spring Security's filter chain runs before Spring MVC's own `ProblemDetail` Jackson support applies; explicitly `@Import`ed per service, never registered in `AutoConfiguration.imports` — its mere presence on a classpath triggers Spring Boot's own default security lockdown, confirmed the hard way against `recycler-service`) + `auth-service`'s own `SecurityConfig` (permits `/auth/login`, Swagger, actuator health; authenticates everything else)
- [x] Task 63: Login API (`JwtIssuer` wrapping Nimbus's `JWSSigner`, `POST /auth/login`, `AuthErrors.INVALID_CREDENTIALS`/`AUTH-001` — same error whether the email doesn't exist or the password is wrong)

### Checkpoint 31: auth-service core proven
- [x] `mvn -pl auth-service verify` green
- [x] Manual check: `docker compose --env-file .env.local up -d`, set `ADMIN_BOOTSTRAP_EMAIL`, boot `auth-service`, retrieve the logged bootstrap password from the startup log, `curl POST /auth/login`, get a real JWT back, decode it (e.g. jwt.io or a one-line script) and confirm `sub`/`email`/`exp` claims are present and correct — confirmed live: `sub` a real staff-user UUID, `email` correct, `exp - iat` exactly 3600s
