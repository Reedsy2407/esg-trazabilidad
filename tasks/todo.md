...
- [ ] Task 36: `CertificationStatusEventListener`
  ...
  - **Acceptance criteria:**
    ...
    - [ ] **User-flagged finding to resolve here, not deferred:** `BlockedAssociationRepositoryAdapter.block()` (Task 35) is a check-then-act (`existsById()` → choose `new` vs `existing()`) — the exact pattern `CertificationStatusLedgerEntity`'s own Javadoc warns against as racy under concurrency. Task 35's re-block test only proves 2 *sequential* calls, not concurrent ones. If this listener's container concurrency can be >1, two threads racing to block the *same* `associationId` for the first time can both see `existsById() == false` and both attempt an insert, so `block()`'s call site here needs a `try/catch DataIntegrityViolationException` around it (same short-circuit pattern used everywhere else in this codebase for TOCTOU races) to swallow the loser's duplicate-key failure. If listener concurrency is instead pinned to 1 (e.g. `SimpleMessageListenerContainer` default, or an explicit `spring.rabbitmq.listener.simple.concurrency=1`), this race can't happen — but that decision must be made and written down explicitly in this task's completion notes, not left implicit
