 ...
 * here so this test stays deterministic and isolated (same reasoning as
 * collection-service's Task 27).
 *
...
class OutboxEventRepositoryAdapterIT {
    ...
    void tiedCreatedAtOrdersByIdAsATiebreaker() {
        // Same fix as collection-service's Task 27: id is a UUID v7 tiebreaker
        // for two rows written in the same instant, required by Task 39's
        // ordering guarantee. Deliberately saved out of id order (largerId
        // first) so a query without the tiebreaker would come back in
